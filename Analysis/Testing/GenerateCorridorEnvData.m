...
%Left Wall
points = [points; GenerateQuadrilateralPoints([left, front, bottom], ...
    [0, depth, 0], [0, 0, height], yDensity * 0.5, zDensity)];
...
points = [points; GenerateQuadrilateralPoints([left, front, top], ...
    [widthBy3, 0, 0], [0, depth, 0], xDensityBy3 * 0.5, yDensity * 0.5)];
%Middle
points = [points; GenerateQuadrilateralPoints( ...
    [left + widthBy3, front + depthBy3, top], ...
    [widthBy3, 0, 0], [0, depthBy3, 0], ...
            xDensityBy3 * 0.5, yDensityBy3 * 0.5)];
%Right
...
points = [points; GenerateQuadrilateralPoints( ...
    ...
    [0, depthBy3, 0], [0, 0, height], yDensityBy3, zDensity)];
points = [points; GenerateQuadrilateralPoints( ...
...
points = [points; GenerateQuadrilateralPoints( ...
    [right - widthBy3, front, bottom + 1.5 * proximityTolerance], ...
    [0, depthBy3, 0], [0, 0, height], yDensityBy3, zDensity)];
points = [points; GenerateQuadrilateralPoints( ...
    [right - widthBy3, back, bottom + 1.5 * proximityTolerance], ...
    [0, -depthBy3, 0], [0, 0, height], yDensityBy3, zDensity)];
